...
function ensureDbExists() {
  try {
    ...
    console.error('Error ensuring DB existence:', error);
  }
}
...
export async function getData(collection: 'users' | 'events' | 'winners' | 'settings') {
  ...
  try {
    ...
    return db[collection] || (collection === 'settings' ? {} : []);
  } catch (error) {
    console.error(`Error reading ${collection} from DB:`, error);
...
export async function cleanupEventWinners(eventId: string) {
  ensureDbExists();
  try {
    const fileData = fs.readFileSync(DB_PATH, 'utf-8');
    const db = JSON.parse(fileData);
    db.winners = (db.winners || []).filter((w: any) => w.event_id !== eventId);
    fs.writeFileSync(DB_PATH, JSON.stringify(db, null, 2));
  } catch (error) {
    console.error(`Error cleaning up winners for event ${eventId}:`, error);
  }
}
